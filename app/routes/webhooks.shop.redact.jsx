...
export const action = async ({ request }) => {
  ...
  try {
    ...
    // Try parsing body just for logging
    try {
      const body = JSON.parse(rawBody);
      ...
      const { shop_id, shop_domain } = body;
      if (!shop_id || !shop_domain) {
        console.warn("⚠️ Missing shop_id or shop_domain, but acknowledging.");
      } else {
        console.log(`🗑 Redact request received for shop ${shop_domain} (${shop_id}).`);
      }
    } catch {
      console.log("⚠️ Could not parse JSON body, but continuing...");
    }

    console.log("ℹ️ No shop data stored in app → nothing to delete.");
    ...
    console.log("🏁 [Webhook] shop/redact finished");
  }
};
